...
##Explanation
to_markdown():
A helper function designed to format the text output from the Gemini model into Markdown syntax, so it displays neatly in the Streamlit interface.
replace('•', ' *'):
This line replaces bullet points (•) from the raw Gemini output with Markdown-compatible list bullets (*). This allows the list items to render correctly in Markdown format.
textwrap.indent():
This indents each line of the text using the > symbol, turning it into a blockquote in Markdown. This gives the output a clean, indented look in the Streamlit app.
